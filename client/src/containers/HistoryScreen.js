...
class HistoryScreen extends Component {
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={s.header}>
          <TouchableOpacity onPress={() => alert('이미지 수정하기')}>
            <Image style={s.profileImage} source={require('../../img/my_page.png')}/>
            <Image style={s.profileImagePlus} source={require('../../img/add_profile.png')}/>
          </TouchableOpacity>

          <View style={{ marginLeft: 21 }}>
            <Text style={createText('bold', 'white', 18)}>{name} 님의 히스토리</Text>
            <Text style={[createText('default', 'white', 14), { marginTop: 8 }]}>
              총 {this.props.selectLocationList.length}개의 여정 히스토리가 존재합니다
            </Text>
          </View>
        </View>
